...
export default function LotMap({ lots, amenities }: LotMapProps) {
  ...
  return (
    <div className="lg:h-full lg:flex lg:flex-col lg:overflow-hidden">
      ...
      <header className="sticky top-0 z-30 flex-shrink-0 bg-white border-b border-neutral-100">
        ...
        <div className="px-3 sm:px-6 lg:px-8 py-2 sm:py-3">
          <div className="max-w-screen-2xl mx-auto flex flex-wrap items-center gap-x-4 sm:gap-x-6 gap-y-2">
            {/* Filtros */}
            <div className="flex items-center gap-2">
              <span className="hidden sm:inline text-[10px] font-medium uppercase tracking-wider text-neutral-400">Filtrar</span>
              <div className="flex gap-1">
                {([
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 sm:py-1 text-[11px] font-medium text-center transition-colors ${
    ...
    <div className="flex flex-col lg:flex-row gap-0 lg:gap-4 px-2 py-2 sm:px-6 sm:py-4 lg:px-8 lg:flex-1 lg:min-h-0 lg:overflow-hidden">
      {/* Map area */}
      <div className="relative flex-1 min-w-0 max-w-screen-2xl mx-auto w-full lg:h-full">
        ...
        <div className="absolute top-2 right-3 sm:right-5 z-20 flex flex-col gap-1">
          <button
            onClick={() => saveCenterAndZoom((z) => Math.min(z + 0.1, 3))}
            className="w-9 h-9 sm:w-7 sm:h-7 bg-neutral-900 text-white hover:bg-neutral-700 flex items-center justify-center text-sm font-medium transition-colors shadow-sm"
            aria-label="Acercar"
          >
            +
          </button>
          <button
            ...
            disabled={zoom <= 1}
            className="w-9 h-9 sm:w-7 sm:h-7 bg-neutral-900 text-white hover:bg-neutral-700 flex items-center justify-center text-sm font-medium transition-colors shadow-sm disabled:opacity-30 disabled:cursor-default"
            aria-label="Alejar"
          >
            −
          </button>
          {zoom > 1 && (
            <button
              onClick={() => setZoom(1)}
              className="w-9 h-9 sm:w-7 sm:h-7 bg-neutral-900 text-white hover:bg-neutral-700 flex items-center justify-center transition-colors shadow-sm"
    ...
    <footer className="flex-shrink-0 border-t border-neutral-100">
      <div className="max-w-screen-2xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-5 flex flex-wrap items-center justify-between gap-x-6 gap-y-2">
        <div className="flex items-center gap-3">
          <a href="/admin" className="p-2 -m-2 text-neutral-300 hover:text-neutral-500 transition-colors" aria-label="Admin">
          ...
          <span className="text-[11px] sm:text-xs text-neutral-400">
            © {new Date().getFullYear()} Mirador Alto Colbún
          </span>
        </div>
        <div className="flex items-center gap-1 sm:gap-4">
          <a href="https://wa.me/56966298663" target="_blank" rel="noopener noreferrer" className="hidden sm:inline text-xs text-neutral-400 hover:text-neutral-900 transition-colors">
            +56 9 6629 8663
          </a>
          <div className="hidden sm:block w-px h-3.5 bg-neutral-100" />
          <a href="https://wa.me/56966298663" target="_blank" rel="noopener noreferrer" className="p-2 text-neutral-400 hover:text-neutral-900 transition-colors" aria-label="WhatsApp">
            <svg className="w-5 h-5 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
          </a>
          <a href="https://www.instagram.com/miradoraltocolbun" target="_blank" rel="noopener noreferrer" className="p-2 text-neutral-400 hover:text-neutral-900 transition-colors" aria-label="Instagram">
          ...
          <a href="https://miradoraltocolbun.cl" target="_blank" rel="noopener noreferrer" className="p-2 text-neutral-400 hover:text-neutral-900 transition-colors" aria-label="Sitio web">
            <svg className="w-5 h-5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/></svg>
          </a>
        </div>
      </div>
    </footer>
